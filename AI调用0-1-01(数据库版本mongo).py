...
db = client['ai_calls_db']
user_calls_collection = db['user_calls']

def init_db():
    # 确保索引已创建
    user_calls_collection.create_index([('user_id', 1)], unique=True)
    # 初始化管理员密钥
    admin_doc = db.admin.find_one()
    if not admin_doc:
        db.admin.insert_one({'admin_key': 'your_admin_key'})
...
class AIInterface:
    def __init__(self, config_file):
        ...
        self.user_calls = user_calls_collection
    ...
    def check_user_calls(self, user_id):
        user = self.user_calls.find_one({'user_id': user_id})
        if not user:
            # 初始化新用户
            self.init_user(user_id)
            return True, self.max_calls_per_user
        remaining_calls = user.get('remaining_calls', 0)
    ...
    def decrease_user_calls(self, user_id):
        ...
        self.user_calls.update_one(
            {'user_id': user_id},
            {'$set': {'last_call_time': datetime.now().isoformat()}}
        )

    def reset_user_calls(self, user_id):
        self.user_calls.update_one(
            {'user_id': user_id},
            {'$set': {'remaining_calls': self.max_calls_per_user, 'last_call_time': datetime.now().isoformat()}}
        )
