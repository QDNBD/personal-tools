...
soapa_logging_v2.simple_config("redis-memory-usage-mq")
logger = logging.getLogger("redis-memory-usage-mq")
SCAN_COUNT = 10000


class Redis(object):
    def __init__(self, scan_match):
        self.redis_cli = RedisDao.get(target="redis.redis-mq", db=11,
                                      socket_timeout=30)
        # self.redis_cli = RedisUtil.get_redis_cluster_nta_mdp_db(socket_timeout=30)
...
if __name__ == '__main__':
    ...
    logger.info(
        "total scan count: {}, key_count_map: {}, key_memory_map: {}".format(
            ...
            key_count_map_,
            key_memory_map_))
    logger.info("all time seconds: {}".format(time.time() - ts))
